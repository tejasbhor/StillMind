...
export default function RegisterPage() {
  ...
  const onInitiateSubmit = async (data: RegisterData) => {
    ...
    try {
      await authApi.initiateStudentRegistration({
        email: data.email,
      ...
      if (msg.includes("restricted to")) {
        setError(`${msg} Please use your institutional Google account to sign up instead.`);
  ...
  const onVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authApi.verifyStudentRegistration(registrationData.email, verificationCode);

      // Successfully registered, now login and submit consents
      await login({ email: registrationData.email, password: registrationData.password });

      await studentApi.submitConsents({
        data_usage: { granted: registrationData.dataUsageConsent },
        counseling: { granted: registrationData.counselingConsent },
        emergency_escalation: { granted: !!registrationData.emergencyEscalationConsent },
      });

      router.push("/dashboard/assessment");
    } catch (e: any) {
      setError(e?.message || "Verification failed. Please check your code.");
    } finally {
      setLoading(false);
    }
  };
      ...
      {step === 1 ? (
        <>
          ...
          {selectedRole !== "student" ? (
            ...
            <form onSubmit={form.handleSubmit(onInitiateSubmit)} className="flex flex-col gap-5" noValidate>
              {error && (
        ...
        <form onSubmit={onVerifySubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            <Button type="submit" size="lg" className="w-full !rounded-full !py-4" loading={loading} disabled={verificationCode.length !== 6}>
              Complete registration
            </Button>
            <button
              type="button"
              onClick={() => setStep(1)}
              className="font-sans text-xs font-bold uppercase tracking-widest text-teal/40 hover:text-teal transition-colors py-2"
            >
